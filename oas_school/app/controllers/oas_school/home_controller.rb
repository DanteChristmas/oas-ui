module OasSchool
  class HomeController < OasSchool::ViewController
    include OasSchool::HeroHelper
    include OasSchool::HomeHelper
    ...
    def index
      ...
      if !@super_override || !@super_override.length || !(@super_override.length > 0)
        @superhero = get_hero({ limit: 4, content_types: ['article', 'media'], has_priority: true })
      end
